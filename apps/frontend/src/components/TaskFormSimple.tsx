...
// Inner form component that gets fresh props each time
const TaskFormInner: React.FC<{ task?: Task; onClose: () => void }> = ({ task, onClose }) => {
...
// Main component that handles modal and conditional rendering
const TaskFormSimple: React.FC<TaskFormProps> = ({ task, isOpen, onClose }) => {
  const isEditing = !!task;
  ...
  return (
    ...
    >
      <TaskFormInner 
        key={task?.id || 'new'} // Force remount on task change
        task={task} 
        onClose={onClose} 
      />
    </Modal>
